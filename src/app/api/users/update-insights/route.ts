 ...
 * 
 * POST /api/users/update-insights
 * Body: { email: string, insightsLeft: number }
 */
export async function POST(req: NextRequest) {
  try {
    await dbConnect();
    console.log('POST /api/users/update-insights - Connected to MongoDB');
    ...
    if (!email) {
      console.log('POST /api/users/update-insights - No email provided');
    ...
    if (typeof insightsLeft !== 'number') {
      console.log('POST /api/users/update-insights - Invalid insightsLeft value');
      return NextResponse.json(
    ...
    console.log(`POST /api/users/update-insights - Updating insights count for user: ${email} to ${insightsLeft}`);
    
    // Find user by email
    const user = await User.findOne({ email });
    
    if (!user) {
      console.log(`POST /api/users/update-insights - No user found with email: ${email}`);
      return NextResponse.json(
 ...
 * GET /api/users/update-insights?email=user@example.com&count=10
 */
export async function GET(req: NextRequest) {
  try {
    await dbConnect();
    console.log('GET /api/users/update-insights - Connected to MongoDB');
    ...
    if (!email) {
      console.log('GET /api/users/update-insights - No email provided');
      return NextResponse.json(
    ...
    if (!countStr) {
      console.log('GET /api/users/update-insights - No count provided');
      return NextResponse.json(
    ...
    if (isNaN(insightsLeft)) {
      console.log('GET /api/users/update-insights - Invalid count value');
      return NextResponse.json(
    ...
    console.log(`GET /api/users/update-insights - Updating insights count for user: ${email} to ${insightsLeft}`);
    
    // Find user by email
    const user = await User.findOne({ email });
    
    if (!user) {
      console.log(`GET /api/users/update-insights - No user found with email: ${email}`);
      return NextResponse.json(
        { success: false, message: 'User not found' },
        { status: 404, headers: corsHeaders }
      );
