 ...
 * GET /api/v1/users/email?email=user@example.com
 */
export async function GET(req: NextRequest) {
  try {
    await dbConnect();
    console.log('GET /api/v1/users/email - Connected to MongoDB');
    ...
    if (!email) {
      console.log('GET /api/v1/users/email - No email provided');
    ...
    console.log(`GET /api/v1/users/email - Looking up user with email: ${email}`);
    
    // Find user by email
    const user = await User.findOne({ email }).select('-password');
    
    if (!user) {
      console.log(`GET /api/v1/users/email - No user found with email: ${email}`);
      return NextResponse.json(
        { 
          success: false, 
    ...
    console.log(`GET /api/v1/users/email - Found user with ID: ${user.id}`);
    ...
    try {
      ...
      if (company) {
        console.log(`GET /api/v1/users/email - Found company: ${company.name} with assistant_id: ${company.assistant_id}`);
