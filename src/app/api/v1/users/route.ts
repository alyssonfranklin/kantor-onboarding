...
export async function GET(req: NextRequest) {
  return withAuth(req, async (req, user) => {
    console.log('GET /api/v1/users - Handler started with user:', {
      id: user.id, 
      role: user.role, 
    ...
    try {
      await dbConnect();
      console.log('GET /api/v1/users - Connected to MongoDB');
      ...
      console.log('GET /api/v1/users - Query parameters:', { companyId, email, role, limit, skip });
      ...
      if (email) {
        ...
        console.log(`GET /api/v1/users - Searching for user with email: ${email}`);
        query = { ...query, email };
      }
      ...
      console.log('GET /api/v1/users - Final query:', query);
      ...
      console.log(`GET /api/v1/users - Found ${users.length} users out of ${total} total`);
